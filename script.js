...
function addToCart() {
    if (document.getElementById('counter-amount').innerText === '0') {
        ...
        cartItem.innerHTML = (`
            ...
            <div>
                <p>${name}</p> 
                <p>$${price} x ${document.getElementById('counter-amount').innerText} 
                    <span class = "navigation--accountmenu--cart--item_total">$${(price * document.getElementById('counter-amount').innerText).toFixed(2)}</span>
                </p>
            </div>
            <svg onclick = "deleteItem(event)" width="14" height="16" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                <defs>
